...
"(eval\s*\(|assert\s*\(|system\s*\(|shell_exec\s*\(|passthru\s*\(|popen\s*\(|proc_open\s*\(|base64_decode\s*\(|gzinflate\s*\(|gzuncompress\s*\(|str_rot13\s*\(|urldecode\s*\(|rawurldecode\s*\(|pack\s*\(|move_uploaded_file|is_uploaded_file|file_get_contents\s*\(|file_put_contents\s*\(|curl_init\s*\(|\$_POST\s*\[\s*['\"]password['\"]\s*\]|egre55|pages\.dev|\$_(POST|GET|REQUEST|COOKIE|FILES))" \
...
# (split strings & concat abuse)
# =========================

echo
echo -e "${PURPLE}[*] Obfuscation heuristic scan (string-split tricks)...${NC}"
